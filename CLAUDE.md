...
- **Hooks**: PostToolUse NEVER sets `updatedMCPToolOutput` (per anthropics/claude-code#36843). PreToolUse sets `updatedInput` ONLY for RTK rewrite (exit 0/3); budget block always wins over RTK rewrite. **Hooks require `npm install -g @cocaxcode/token-optimizer-mcp`** — without global install, `npx` can't find the binary and hooks fail silently. Session restart required after install.
